...
test("map page uses the standard left-side controls with zoom first", async () => {
  ...
  assert.match(html, /type="module" src="\.\/app\.js\?v=0\.8\.13"/);
  ...
  assert.match(await readFile(new URL("../../public/plotter/ajrm-map-core.mjs", import.meta.url), "utf8"), /No Auto chart — basemap shown/);
  ...
  assert.match(await readFile(new URL("../../public/plotter/ajrm-map-core.css", import.meta.url), "utf8"), /width:min\(92vw,48rem\)/);
